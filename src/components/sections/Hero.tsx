...
export function Hero() {
  return (
    ...
    >
      ...
      />
      <div
        className="absolute inset-0 bg-[radial-gradient(ellipse_70%_60%_at_85%_35%,rgba(232,185,35,0.18),transparent_55%)]"
        aria-hidden
      />
      <div
        className="absolute inset-0 bg-[radial-gradient(ellipse_50%_45%_at_15%_85%,rgba(245,208,58,0.1),transparent_50%)]"
        aria-hidden
      />
      ...
      <Container className="relative z-10">
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-10 lg:gap-x-14">
          <div className="text-center lg:text-left">
            <p className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-medium text-amber-100 shadow-[0_0_24px_rgba(245,208,58,0.12)] backdrop-blur-md">
              <Cpu className="size-3.5 text-[var(--brand-yellow-bright)]" aria-hidden />
            ...
            <div className="mt-10 flex flex-col items-stretch justify-center gap-3 sm:flex-row sm:flex-wrap lg:justify-start">
              <Button
                asChild
                size="lg"
                className="w-full border-0 bg-[var(--brand-yellow)] text-[var(--brand-dark)] shadow-[0_0_32px_rgba(232,185,35,0.35)] hover:bg-[var(--brand-yellow-bright)] sm:w-auto"
              >
                <Link href={`/?intent=equipment#${homeAnchors.leadForm}`}>Подобрать кассу</Link>
              </Button>
              <Button
                asChild
                size="lg"
                variant="outline"
                className="w-full border-2 border-white/35 bg-white/5 text-white backdrop-blur-sm hover:bg-white/12 hover:text-white sm:w-auto"
              >
                <Link href={`/?intent=consultation#${homeAnchors.leadForm}`}>Получить консультацию</Link>
              </Button>
            </div>
          </div>
          ...
          <div className="relative mx-auto w-full max-w-md lg:max-w-none lg:justify-self-end">
            <div
              className="pointer-events-none absolute inset-0 -z-10 scale-110 rounded-full bg-[radial-gradient(circle_at_50%_50%,rgba(245,208,58,0.22),transparent_68%)] blur-2xl"
              aria-hidden
            />
